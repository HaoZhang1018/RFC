...
class CLIPDenseBase(nn.Module):
    ...
    def compute_conditional(self, conditional):
        ...
        if type(conditional) in {list, tuple}:
            text_tokens = clip.tokenize(conditional).to(device)
            cond = self.clip_model.encode_text(text_tokens)
        else:
            if conditional in self.precomputed_prompts:
                cond = self.precomputed_prompts[conditional].float().to(device)
            else:
                text_tokens = clip.tokenize([conditional]).to(device)
    ...
    def get_embedding(self, process):
        device = next(self.parameters()).device
        process = clip.tokenize(process).to(device)
        process_embedding = self.clip_model.encode_text(process)  # [b 512]
...
class CLIPDensePredT(CLIPDenseBase):
    ...
    def forward(self, inp_image, conditional=None, return_features=False, mask=None, h=480, w=640):
        ...
        if conditional == ['image', 'image']:
            # print('seg image')
            seg_ones = torch.ones(bs, 1, h, w).to(dev)
            # ones_fea = torch.ones(bs, 64, 22, 22).to(dev)

            visual_q, activations, _ = self.visual_forward(x_inp, extract_layers=[0] + list(self.extract_layers))
            activations = activations[1:]
            _activations = activations[::-1] if not self.rev_activations else activations
            clip_out = self.reduces[0](_activations[0])
            clip_out = clip_out[1:].permute(1, 2, 0)
            size = int(math.sqrt(clip_out.shape[2]))
            clip_out = clip_out.view(bs, 64, size, size)
            # print(f'clip_out:{clip_out.shape}')
            return seg_ones, clip_out
        elif conditional == [0, 0]:
            # print('no seg')
            seg_zero = torch.zeros(bs, 1, h, w).to(dev)
            zero_fea = torch.zeros(bs, 64, 22, 22).to(dev)
            return seg_zero, zero_fea
        else:
            ...
            # add code
            #clip_out = clip_out[1:].permute(1, 2, 0)
            #clip_out = clip_out.view(bs, a.shape[1], size, size)
            clipseg_fea = a.clone()
